...
var MarkerController = function () {
    ...
    _createClass(MarkerController, [{
        ...
        key: 'setClusterBalloonState',
        value: function setClusterBalloonState(state, cluster) {
            if (state === 'opened') {
                var geoObjectState = cluster.getObjectState(this._marker);
                if (geoObjectState.isShown) {
                    if (geoObjectState.isClustered) {
                        geoObjectState.cluster.state.set('activeObject', this._marker);
                        cluster.balloon.open(geoObjectState.cluster);
                    } else {
        ...
        key: 'destroyOnCluster',
        value: function destroyOnCluster(cluster) {
            this.events.removeAll();
